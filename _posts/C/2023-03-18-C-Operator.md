...
## 📚산술 연산자
### 📄이항 연산자
2개의 자료를 대상으로 산술적인 처리를 수행하는 연산자이다.
...
### 📄단항 연산자
1개의 자료만을 대상으로 산술적인 처리를 수행하는 연산자이다. ++, -- 연산자를 증감 연산자라고도 한다. 피연산자의 값을 1 증가시키거나 1 감소시킨다. 증감 연산자는 사용 위치에 따라 기능이 달라질 수 있다.
<br>
...
* 전위연산
<br>
++i와 같이 피연산자 앞에 연산자를 붙인다. i 값을 먼저 사용한 후 1 증가시킨다.
<br>

* 후위연산
<br>
i++와 같이 피연산자 뒤에 연산자를 붙인다. i 값을 먼저 1 증가시킨 후 사용한다.
...
## 📚관계 연산자
관계 연산자는 피연산자의 대소 관계를 비교하는 연산자이다. 결과는 참과 거짓이 된다.
...
## 📚논리 연산자
논리 연산자는 피연산자에 대해 논리연산을 수행하는 연산자이다. 결과는 참과 거짓으로 나타난다. 값이 0인 경우 거짓이고 0이 아닌 값은 참이 된다.
...
## 📚대입 연산자
대입 연산자는 연산자의 오른쪽을 왼쪽에 대입하는데 사용한다.
...
## 📚조건 연산자
조건 연산자는 조건의 만족 여부에 따라 지정된 수식을 수행하는 연산자이다. 3개의 피연산자를 가지고 있어 3항 연산자이다.
...
## 📚비트 연산자
비트 연산자는 수치를 2진수로 변환하여 bit 단위의 연산을 수행하는 연산자이다. 비트 연산자는 정수형 자료에서만 사용이 가능하다.
...
<br>
형변환에는 자동 형변환과 강제 형변환이 있다. 대입 연산자의 왼쪽과 오른쪽에 존재하는 두 피연산자의 자료형이 불일치하거나, 산술연산에서 2개의 피연산자의 자료형이 불일치하는 경우에 자동 형변환이 일어난다.
...
## 📚연산자 우선순위
수학과 마찬가지로 C 언어에서도 연산자 우선 순위가 정해져 있다. 하나의 연산식에 여러 개의 연산자가 사용될 경우 연산자 우선순위에 의해 연산이 이루어진다. 우선순위가 같은 연산자가 있을 경우 연산자의 결합 방향에 의해 연산이 이루어진다. 괄호를 사용하여 연산을 먼저 수행하게 하는 방법도 있다.

우선순위와 결합 방향에 대한 자세한 내용은 아래 링크를 참고
